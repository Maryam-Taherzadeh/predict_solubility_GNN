...
col1, _ = st.columns([1, 5])
with col1:
    molecule_name = st.text_input("🔍 Enter Molecule Name *or* SMILES", "")
...
if st.button("✨ Predict Solubility"):
    if is_smiles(molecule_name):
        smiles = molecule_name.strip()
    else:
        url = f"https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/{molecule_name}/property/CanonicalSMILES/TXT"
        response = requests.get(url)
        smiles = response.text.strip() if response.status_code == 200 else None
...
""", unsafe_allow_html=True)
        ...
        st.warning("❌ Molecule not found in PubChem or invalid SMILES.")
